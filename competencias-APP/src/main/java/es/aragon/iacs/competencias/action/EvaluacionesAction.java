    	...
    	for(int i=0;i<todasEvaluaciones.size();i++) {
    		...
    		if(todasEvaluaciones.get(i).getCatcompetencial()!= null && todasEvaluaciones.get(i).getCatcompetencial().equals(catCompetencial)) {
    			...
    			if (listaEvaluaciones.size()!=0 && listaEvaluaciones.get(listaEvaluaciones.size()-1).getId() !=todasEvaluaciones.get(i).getId()) {
        			...
        			} //BUCLE CON LOS EVALUADORES INTERNOS  SI NO SE HA A�ADIDO YA
    				if (listaEvaluaciones.size()!=0 && listaEvaluaciones.get(listaEvaluaciones.size()-1).getId() !=todasEvaluaciones.get(i).getId()) {
...
//    					
    	...
    	mis=true;
    	listaCatCompetenciales=catCompetencialesDao.findActivas();
...
//        editar=false;
    ...
    public String nueva() {
    	...
    	//COMPROBAR QUE PUEDE A�ADIRSE: CATCOMPETNECIAL NO ESTA SINEDO EVALUADA YA DURANTE ESE PERIODO
	...
	//        listaEvaluaciones=evaluacionesDao.findAll();
    	...
    	else { //NO SE HA PODIDO A�ADIR
    		log.debug("no se ha podido a�adir idNueva: "+idNueva);
    		listaCatCompetenciales=catCompetencialesDao.findActivas();
        	listaTrabajadores = trabajadoresDao.findAll();
            listaEvaluaciones=evaluacionesDao.findAll();
            objCompCatcomp=competenciasDao.allObjCompCatcomp();
...
//    	log.debug("PROBANDO COMPET: "+compet);
    ...
    public String concreta() {
    	...
    	// buscar de tus pares, superiores, inferiores, etc quienes tienen catcompetnecial y a�adirlos a la lista de trabjaadores a evaluar
...
//    	meHanElegido=new ArrayList<CompEvaluadorInterno>();
    	...
    	//COMPRPBAR EN LISTA DE EVALUADORES INTERNOS, Y A�ADIR AQUELLOS DE LOS CUALES TU SEAS EVALUADOR
