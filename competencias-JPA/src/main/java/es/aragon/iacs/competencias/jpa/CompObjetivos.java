...
@NamedQueries({
	...
	@NamedQuery(name="CompObjetivos.findById", query="SELECT c FROM CompObjetivos c WHERE c.id=:id ORDER BY c.id"),
	@NamedQuery(name="CompObjetivos.findRelacion", query="SELECT c FROM CompObjetivos c WHERE c.codcompetencia=:codcompetencia AND c.codcatcomp=:codcatcomp ORDER BY c.id")
